...
class PianoTiles:
    ...
    def _is_tile(self,pixel,threshold):
        ...
        return True if color[0] <= threshold and color[0] >= threshold - 10 else False
 
    def run(self, *, tile_rgb = 10, tile_rgb2 = 255):
        while not keyboard.is_pressed('esc'):
            for pos in self.tiles:
                if self._is_tile(pos, tile_rgb) or self._is_tile(pos, tile_rgb2):
                    pyautogui.click(*pos)
                    break
